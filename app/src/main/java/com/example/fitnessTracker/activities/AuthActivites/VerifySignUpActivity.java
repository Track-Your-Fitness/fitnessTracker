...
public class VerifySignUpActivity extends AppCompatActivity {
    ...
    public void setUpConfirmButton() {
        findViewById(R.id.verifySignUpActivitySubmitButton).setOnClickListener(view -> {
            String userEmail = "";
            if (callingIntent != null) {
                userEmail = callingIntent.getStringExtra(SignUpActivity.USER_EMAIL);
            }
            String confirmationCode = ((EditText) findViewById(R.id.verifySignUpActivityConfirmationCodeEditText)).getText().toString();
